...
public final class DropDown: UIView {
    ...
    override public func draw(_ rect: CGRect) {
        super.draw(rect)
        guard let windowFrame = anchorView?.plainView.windowFrame else { return }
        ...
        context.setShadow(offset: shadowOffset, blur: CGFloat(shadowOpacity*30.0), color: shadowColor.withAlphaComponent(0.3).cgColor)
        
        var arror_height:CGFloat = tableViewContainer.frame.width/4
        arror_height = arror_height > 10 ? 10:arror_height
        var midx = windowFrame.midX
        if midx + arror_height > tableViewContainer.frame.maxX - 10 {
            midx = tableViewContainer.frame.maxX - 10 - arror_height
        }
        let radius:CGFloat = cornerRadius
        let minx = tableViewContainer.frame.minX
        let maxx = tableViewContainer.frame.maxX
        let miny = tableViewContainer.frame.minY
        let maxy = tableViewContainer.frame.maxY
        //箭头在下方
        if tableViewContainer.frame.minY < windowFrame.minY{
            context.move(to: CGPoint(x:  midx+arror_height, y: maxy))
            context.addLine(to: CGPoint(x:  midx, y: maxy+arror_height))
            context.addLine(to: CGPoint(x:  midx-arror_height, y:maxy))
        ...
        else if tableViewContainer.frame.minY > windowFrame.minY{//箭头在上方

            context.move(to: CGPoint(x:  midx+arror_height, y: miny))
            context.addLine(to: CGPoint(x:  midx, y: miny - arror_height))
            context.addLine(to: CGPoint(x:  midx-arror_height, y:miny))
            context.addArc(tangent1End:  CGPoint(x:  minx, y:miny), tangent2End:  CGPoint(x: minx, y:maxy), radius: radius)
            context.addArc(tangent1End:  CGPoint(x:  minx, y:maxy), tangent2End:  CGPoint(x: maxx, y:maxy), radius: radius)
            context.addArc(tangent1End:  CGPoint(x:  maxx, y:maxy), tangent2End:  CGPoint(x: maxx, y:miny), radius: radius)
            context.addArc(tangent1End:  CGPoint(x:  maxx, y:miny), tangent2End:  CGPoint(x: midx+arror_height, y:miny), radius: radius)
            context.closePath()
            context.drawPath(using: .fillStroke)
        }
    }
...
extension DropDown {
    ...
    fileprivate func setupConstraints() {
        ...
        tableViewContainer.addUniversalConstraints(format: "|-0-[tableView]-0-|", views: ["tableView": tableView])
    }
    ...
    fileprivate func computeLayoutBottomDisplay(window: UIWindow) -> ComputeLayoutTuple {
        ...
        y = y + (anchorView?.plainView.bounds.height ?? 0)
...
extension DropDown {
    ...
    public func show() -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
        ...
        if self == DropDown.VisibleDropDown,!self.isHidden {
            return (true, 0)
        }
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
    ...
    fileprivate func configureCell(_ cell: DropDownCell, at index: Int) {
        ...
        cell.selectTextColor = selectionTextColor
